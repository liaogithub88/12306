...
const fieldDefinitions = {
    ...
    'dingding': [
        {
            key: 'DD_MODE', label: '推送方式', type: 'select',
            options: [
                { label: '群机器人（发送到群聊）', value: 'robot' },
                { label: '应用机器人（发到个人对话）', value: 'app' },
            ],
            help: '「应用机器人」需在钉钉开放平台 → 企业内部开发 → 机器人 创建并发布机器人，机器人即可向指定 userId 的个人会话直接发消息，access_token 会自动刷新；「群机器人」为原有方式，发送到机器人所在群聊',
        },
        { key: 'DD_BOT_TOKEN', label: '机器人 Token', placeholder: '群机器人 Webhook 的 access_token', showWhen: 'robot' },
        { key: 'DD_BOT_SECRET', label: '机器人加签 Secret', placeholder: '加签模式的 Secret', type: 'password', showWhen: 'robot' },
        { key: 'DD_APP_KEY', label: 'AppKey（机器人编码）', placeholder: '机器人应用的 AppKey', showWhen: 'app', help: '企业内部机器人应用的 AppKey，同时用作机器人编码 robotCode' },
        { key: 'DD_APP_SECRET', label: 'AppSecret', placeholder: '机器人应用的 AppSecret', type: 'password', showWhen: 'app' },
        { key: 'DD_USER_IDS', label: '接收人 UserId', placeholder: '例如: zhangsan,lisi', showWhen: 'app', help: '钉钉通讯录中用户的 userId，多个用英文逗号分隔（最多 20 个）；可在钉钉管理后台 → 通讯录中查看' },
    ],
    'wecom_bot': [
...
const syncForm = () => {
    ...
    if (type === 'wecom_app') {
        ...
        if (type === 'dingding' && !currentForm.value['DD_MODE']) {
            currentForm.value['DD_MODE'] = 'robot'
        }
    }
}
...
const isFieldVisible = (field) => {
    ...
    return currentForm.value['DD_MODE'] === field.showWhen
}
