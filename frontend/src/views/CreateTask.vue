...
const queryTrainCodes = async () => {
  ...
  try {
    ...
    console.log('[指定车次] API完整响应:', res)
    console.log('[指定车次] res.data:', res.data)

    const data = res && res.data ? res.data : res
    const trains = data && Array.isArray(data.trains) ? data.trains : []

    console.log('[指定车次] trains:', trains)

    if (trains.length > 0) {
       availableTrainCodes.value = trains
         .map(t => t.train_code)
         .filter(Boolean)
