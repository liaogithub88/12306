...
push_config = {
    ...
    'DD_BOT_SECRET': '',                # 钉钉群机器人的加签 DD_BOT_SECRET（群机器人模式）
    'DD_BOT_TOKEN': '',                 # 钉钉群机器人的 DD_BOT_TOKEN（群机器人模式）

    'DD_MODE': '',                      # 钉钉推送方式：空/robot=群机器人，app=应用机器人（机器人发给个人对话）
    'DD_APP_KEY': '',                   # 钉钉企业内部机器人应用 AppKey，同时用作机器人编码 robotCode（应用机器人模式）
    'DD_APP_SECRET': '',                # 钉钉企业内部机器人应用 AppSecret（应用机器人模式）
    'DD_ROBOT_CODE': '',                # 机器人编码 robotCode，留空默认取 DD_APP_KEY（应用机器人模式）
    'DD_USER_IDS': '',                  # 钉钉接收人 userId，多个用英文逗号分隔，最多 20 个（应用机器人模式）
...
def dingding_bot(title: str, content: str, config=None) -> None:
    """
    使用 钉钉 推送消息，支持两种模式：
    - 群机器人（默认 / DD_MODE=robot）：DD_BOT_TOKEN + DD_BOT_SECRET，发送到机器人所在群聊；
    - 应用机器人单聊（DD_MODE=app）：DD_APP_KEY + DD_APP_SECRET + DD_USER_IDS，
      由企业内部机器人向指定 userId 的个人会话发送消息，access_token 过期自动刷新。
    """
    cfg = config or push_config
    mode = str(cfg.get("DD_MODE") or "").strip().lower()
    if mode == "app":
        _dd_app_send(title, content, cfg)
    else:
        _dd_robot_send(title, content, cfg)


# 钉钉 access_token 进程级缓存（key: app_key），带锁，过期自动刷新
_dingtalk_token_lock = threading.Lock()
_dingtalk_token_cache: dict = {}


def _dd_fetch_access_token(app_key: str, app_secret: str):
    """向钉钉开放平台获取应用 access_token（新版 v1.0 接口）。"""
    try:
        resp = requests.post(
        ...
        print(f"钉钉获取 access_token 请求异常: {exc}")
        return None, 0

    if resp.get("accessToken"):
        return resp["accessToken"], int(resp.get("expireIn") or 7200)

    print(f"钉钉获取 access_token 失败: {resp}")
    return None, 0


def _dd_get_access_token(app_key: str, app_secret: str) -> str:
    """获取钉钉 access_token，进程级缓存 + 过期自动刷新。"""
    now = time.time()
    ...
    token, expires_in = _dd_fetch_access_token(app_key, app_secret)
    if token:
        with _dingtalk_token_lock:
            _dingtalk_token_cache[app_key] = {
                "token": token,
                "expire_at": now + max(expires_in, 60),
            }
    return token


def _dd_invalidate_token(app_key: str):
    """access_token 失效时清除缓存，触发下一次自动刷新。"""
    with _dingtalk_token_lock:
        _dingtalk_token_cache.pop(app_key, None)


def _dd_robot_send(title: str, content: str, cfg: dict) -> None:
    """钉钉群机器人模式：发送到机器人所在群聊。"""
    if not cfg.get("DD_BOT_TOKEN") or not cfg.get("DD_BOT_SECRET"):
        print("钉钉群机器人模式缺少 DD_BOT_TOKEN / DD_BOT_SECRET 配置")
        return
    print("钉钉群机器人 服务启动")
    ...
    if not response.get("errcode"):
        ...
        print(f"钉钉群机器人 推送失败！{response}")


def _dd_app_send(title: str, content: str, cfg: dict) -> None:
    """钉钉应用机器人模式：批量发送人与机器人单聊会话中的机器人消息。"""
    app_key = (cfg.get("DD_APP_KEY") or "").strip()
    app_secret = (cfg.get("DD_APP_SECRET") or "").strip()
    robot_code = (cfg.get("DD_ROBOT_CODE") or "").strip() or app_key
    user_ids = [
        u.strip() for u in (cfg.get("DD_USER_IDS") or "").split(",") if u.strip()
    ]

    if not (app_key and app_secret and user_ids):
        print("钉钉应用机器人模式缺少 DD_APP_KEY / DD_APP_SECRET / DD_USER_IDS 配置")
        return
    if len(user_ids) > 20:
        print(f"钉钉接收人 userId 最多 20 个，当前 {len(user_ids)} 个，已截断")
        user_ids = user_ids[:20]

    print("钉钉应用机器人单聊 服务启动")

    token = _dd_get_access_token(app_key, app_secret)
    if not token:
        return

    # 单聊 markdown 消息：标题上限 20 字，内容上限 5000 字；msgParam 为 JSON 字符串
    msg_param = json.dumps(
        {"title": title[:20], "text": f"### {title}\n\n{content}"[:5000]},
        ensure_ascii=False,
    )
    data = {
    ...
    def _post(target_token: str):
        headers = {
            "Content-Type": "application/json;charset=utf-8",
            "x-acs-dingtalk-access-token": target_token,
        }
    ...
    if response.status_code == 200:
        body = response.json()
        invalid = body.get("invalidStaffIdList") or []
        msg = f"钉钉单聊 推送成功！processQueryKey={body.get('processQueryKey')}"
        if invalid:
            msg += f"，无效 userId: {invalid}"
        print(msg)
        return

    # access_token 失效（HTTP 401）：清缓存自动刷新后重试一次
    if response.status_code == 401:
        print("钉钉 access_token 已失效，自动刷新并重试...")
        _dd_invalidate_token(app_key)
        token = _dd_get_access_token(app_key, app_secret)
        if not token:
            return
        response = _post(token)
        if response.status_code == 200:
            body = response.json()
            print(f"钉钉单聊 重试推送成功！processQueryKey={body.get('processQueryKey')}")
            return
    ...
    try:
        ...
        err = response.text
    print(f"钉钉单聊 推送失败！HTTP {response.status_code}: {err}")
...
def add_notify_function(config=None):
    ...
    dd_mode = str(cfg.get("DD_MODE") or "").strip().lower()
    if dd_mode == "app":
        dd_ready = bool(
            cfg.get("DD_APP_KEY")
            and cfg.get("DD_APP_SECRET")
            and cfg.get("DD_USER_IDS")
        )
    else:
        dd_ready = bool(cfg.get("DD_BOT_TOKEN") and cfg.get("DD_BOT_SECRET"))
    if dd_ready and is_channel_enabled("dingding"):
